...
def extract_patch_and_coord(
    ...
    # D = np.squeeze(np.asarray(coord[int(vix),j].todense()))
    D = np.squeeze(np.asarray(coord[int(vix), : coord.shape[1] // 2].todense()))
    ...
    theta = np.squeeze(np.asarray(coord[int(vix), j_theta].todense()))
